 ...
 * Desc: 获取Druid连接池工具类
 */
public class MyDruidDSUtil {
    private static DruidDataSource druidDataSource;

    public static DruidDataSource createDataSource() {
        if(druidDataSource == null){
            synchronized (MyDruidDSUtil.class){
                if(druidDataSource == null){
                    System.out.println("~~~获取Druid连接池对象~~~");
                    // 创建连接池
                    druidDataSource = new DruidDataSource();
                    // 设置驱动全类名
                    druidDataSource.setDriverClassName(MyGmallConfig.PHOENIX_DRIVER);
                    // 设置连接 url
                    druidDataSource.setUrl(MyGmallConfig.PHOENIX_URL);
                    // 设置初始化连接池时池中连接的数量
