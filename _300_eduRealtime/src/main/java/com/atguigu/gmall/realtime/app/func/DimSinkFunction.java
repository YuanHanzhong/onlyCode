...
public class DimSinkFunction extends RichSinkFunction<JSONObject> {
    ...
    public void invoke(JSONObject jsonObj, Context context) throws Exception {
        ...
        //{"tm_name":"cls","id":12}
        //拼接向phoenix表中插入数据的语句
        String upsertSql = "upsert into " + MyGmallConfig.PHOENIX_SCHEMA + "." + tableName
        ...
        MyPhoenixUtil.executeSql(upsertSql,conn);

        //如果维度数据发生了修改操作，需要将缓存中的维度删除掉
        if("update".equals(type)){
            MyDimUtil.delCached(tableName,jsonObj.getString("id"));
        }
    }
}
